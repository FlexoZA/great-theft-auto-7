...
function Hunt:drawAboveCars()
  ...
  if s then
    Render.squirrel(s, time, self.squirrelHealth)
  end
  local m = self.man
  if m then
    ...
    Render.bar(x, y - 13, 28, math.max(0, m.hp / self.manHealth))
  ...
  local f = self.foot
  if f then
    Render.foot(f, self.leap, time, self.footHealth, self.footRadius)
  end
  love.graphics.setColor(1, 1, 1)
end
