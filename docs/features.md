...
| `serverShopDeliver(server, player, item, n)` | the shop asks, through `Features.any` | A player who is not standing on a bag bought `n` of `item` (not a car). Answer true and the goods are yours to bring them: the shop hands nothing over and tells the buyer SHOP_SENT. The turf war answers with a police car that drives the lane and throws a parcel out (a pickup: `pickups:serverDrop(server, item, x, y, n)` takes any item a bag can hold). |
| `shopAnywhere(client)` / `serverShopAnywhere(server, player)` | the shop asks, through `Features.any` | Answer true and the shop screen stays up away from any bag (client), and the host sells to a buyer who is not standing on one (server); a car still goes to the nearest bag's bays on the map in play. A feature puts a bag on its own map with `Shop:addShop(spec)` and opens the screen from anywhere with `Shop:toggle(client)`. The turf war answers both while the war is on, with a stand in each base and the shop key O. |
| `serverFriendly(server, byId, victim, team)` | weapons asks, through `Features.any` | Is a blow on `victim` from player `byId` (0 or nil for nobody's), or from something of side `team` when given, between friends? Answer true and weapons drops it: a round flies on through them, a blast leaves them out, a ram or a slap does nothing. The turf-war feature answers for the two sides while the war is on. A feature that fires for a side passes it to `weapons:serverFireFrom(..., gun, team)`. |
| `serverPlayerTeam(team, server, player)` | anything that colours or targets by side asks, through `Features.reduce` | The side a player is on (1 or 2) while a team fight is on, nil otherwise. Start from nil; turf-war answers. |
